...
coloc.var.bma <- function(df1,snps=setdiff(colnames(df1),response),
   ...
   ## remove SNPs to ensure that binX is full rank when given an intercept
    ...
    for(i in wh) {
        ...
        this.coloc <-coloc.var.test.summary(coef.1[[i]], coef.2[[i]], var[[i]], plot.coeff=FALSE,bma=TRUE,n.approx=n.approx,bayes.factor=bayes.factor,bayes=bayes,level.ci=0.95,nsnps=nsnps)
	if(bayes) {
            post[i,] <- this.coloc@bma
            p[i,] <- c(this.coloc@result,coloc:::p.value(this.coloc),coloc:::ppp.value(this.coloc))
        } else {
            p[i,] <- c(this.coloc@result,coloc:::p.value(this.coloc))
        }      
        if(length(bayes.factor)) {
    ...
    if(bayes) {
        ...
        ci <- coloc:::credible.interval(post,interval=c(0,pi), n.approx=n.approx)
    }
    if(length(bayes.factor)) {
